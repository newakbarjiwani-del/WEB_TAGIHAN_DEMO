...
8. [Notifikasi sistem (Fase 1)](#8-notifikasi-sistem-fase-1)
...
| `BRAND_PAYMENT_QRIS` | `true` = tombol Top up QRIS; `false` = bayar via VA saja |
...
Tabel penting (minimal untuk QRIS):

- `mst_qris` / item terkait — status pending → paid
- Tabel tagihan / saldo sesuai skema sekolah
- Opsional: `log_qris_push` (diisi handler `demoInstallment.php`)
...
**Tidak ada panggilan Web Push** — notifikasi user hanya dari PWA Fase 1.
...
- [ ] Saat generate QRIS, browser minta izin notifikasi → **Allow**
- [ ] Biarkan PWA terbuka → setelah paid, muncul **notifikasi sistem** + toast
- [ ] Klik notifikasi fokus ke PWA
- [ ] Install “Add to Home Screen” — icon & nama short_name benar
- [ ] Offline: buka tanpa jaringan → `offline.html` (opsional)
...
├── config/brand.php              ← white-label
├── config/services.php           ← WS_TAGIHAN_URL, TAGIHAN_DB_*
├── routes/web.php                ← route UI + manifest + status bayar
├── app/Http/Controllers/
│   ├── TagihanController.php     ← login, VA, QRIS, cek status
│   └── MultiAccountController.php
├── app/Services/
│   ├── QrisGenerateService.php
│   └── MultiAccountService.php
├── resources/views/index3.blade.php  ← UI + paymentWatch + notif Fase 1
├── public/sw.js                  ← service worker PWA
├── public/offline.html
├── public/icons/
├── DEMO_WS_TAGIHAN_CICILAN/      ← rename per client
│   ├── index.php
│   ├── config/database.php
│   ├── config/qris.php
│   ├── controllers/
│   └── models/
└── islamic_center/               ← referensi / deploy ke server callback
    ├── config/connectTagihanCicilan.php
    └── qris/
        ├── pushNotif.php         ← router prefix VA
        └── pushNotif/demoInstallment.php
```
...
2. `composer install` + `.env` (APP_URL, WS, DB, brand, QRIS).  
3. Samakan DB di WS `database.php` + islamic_center connect.  
4. Daftarkan prefix VA di `pushNotif.php` + handler forward.  
5. Deploy Laravel `public/`, WS, handler callback.  
6. Uji login → QRIS/VA → paid di DB → notif saat PWA terbuka.  
